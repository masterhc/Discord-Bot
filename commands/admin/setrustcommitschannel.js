...
const fs = require('fs');
    ...
    class setrustcommitschannelCommand extends commando.Command{
        constructor(client){
            super(client, {
                name: 'setrustcommitschannel',
                group:'admin',
                memberName: 'setrustcommitschannel',
                description: 'Este comando irá usar o canal indicado para mostrar os updates nos repositórios referentes ao jogo Rust. É necessário voltar a usar este comando cada vez que o bot tem uma atialização.'
          ...
          //start channel array var
            ...
            let base = JSON.parse(fs.readFileSync('channels.json', 'utf-8'));
                            ...
                            if (base.channels.length >0){
                                ...
                                for(var i =0; i<base.rustCommitsChannels.length;i++){
                                    basemodel.rustCommitsChannels.push(base.rustCommitsChannels[i]);

                                }
                            }
                            basemodel.rustCommitsChannels.push(args[1])
                            let output = JSON.stringify(basemodel);  
                          
            //Check admin privilege
            if(isAdmin()==true){
                        if(hasArgs()==true){
                            //add rustCommits channel to the list                              
...
}module.exports = setrustcommitschannelCommand;