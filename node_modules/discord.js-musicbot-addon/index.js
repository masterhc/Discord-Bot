...
module.exports = function(client, options) {
  ...
  class Music {
    constructor(client, options) {
      ...
      this.defVolume = parseInt((options && options.defVolume) ||10);
  ...
  musicbot.isAdmin = (member) => {
    ...
    if (musicbot.botAdmins.includes(member.id)) return msg.channel.send(musicbot.note('fail','Então porque não funciona.'));
  ...
  musicbot.musichelp = (msg, suffix) => {
    ...
    if (!suffix) {
      ...
      msg.channel.send(musicbot.note('fail', `${suffix} não é um comando valido!`));
    };
  };
  ...
  musicbot.play = (msg, suffix) => {
    ...
    if (msg.member.voiceChannel === undefined) return msg.channel.send(musicbot.note('fail', `Não está em nenhum canal de voz.`));

    // Make sure the suffix exists.
    if (!suffix) return msg.channel.send(musicbot.note('fail', 'Nenhum video especificado!'));

    // Get the queue.
    const queue = musicbot.getQueue(msg.guild.id);

    // Check if the queue has reached its maximum size.
    if (queue.length >= musicbot.maxQueueSize) return msg.channel.send(musicbot.note('fail', 'Tamanho máximo da lista alcançado !'));
      ...
      .then(response => {
        if (searchstring.startsWith('http') && searchstring.includes("list=")) {
          ...
          ypi.playlistInfo(musicbot.youtubeKey, playid, function(playlistItems) {
            ...
            newItems.forEach(video => {
              ...
              if (ran == newItems.length) {
                if (index = 0) msg.channel.send(musicbot.note('fail', `Não consegui recolher musicas dessa lista de repordução.`))
          ...
          search.search(searchstring, {
            ...
            .then(searchResult => {
              if (!searchResult.totalResults || searchResult.totalResults === 0) return response.edit(musicbot.note('fail', 'Nenhum resultado encontrado.'));
  ...
  musicbot.search = (msg, suffix) => {
    musicbot.dInvoker(msg);
    // Make sure the user is in a voice channel.
    if (msg.member.voiceChannel === undefined) return msg.channel.send(musicbot.note('fail', `Não está num canal de voz.`));
    ...
    // Check if the queue has reached its maximum size.
    if (queue.length >= musicbot.maxQueueSize) return msg.channel.send(musicbot.note('fail', 'A lista atingiu o tamanho máximo!'));
    ...
    msg.channel.send(musicbot.note('search', `Searching: \`${searchstring}\``))
      .then(response => {
        search.search(searchstring, {
          ...
          .then(searchResult => {
            if (!searchResult.totalResults || searchResult.totalResults === 0) return response.edit(musicbot.note('fail', 'Não encontrou resultados.'));

            const startTheFun = async (videos, max) => {
              const embed = new Discord.RichEmbed();
              embed.setTitle(`Escolhe o Video`);
              embed.setColor(0x27e33d);
              var index = 0;
                ...
                .then(firstMsg => {
                  ...
                  msg.channel.awaitMessages(filter, {
                    ...
                    .then(collected => {
                      ...
                      if (mcon === "cancel") return firstMsg.edit(musicbot.note('note', 'Procura canceleda.'));
                      const song_number = parseInt(mcon) - 1;
                      if (song_number >= 0) {
                        firstMsg.delete();
                        return msg.channel.send(musicbot.note('note', `Em lista **${musicbot.note('font', videos[song_number].title)}**`))
                          .then(() => {
  ...
  musicbot.skip = (msg, suffix) => {
    ...
    if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Nenhuma musica em reprodução.'));

    // Get the queue.
    const queue = musicbot.getQueue(msg.guild.id);

    if (!musicbot.canSkip(msg.member, queue)) return msg.channel.send(musicbot.note('fail', `Não colocas-te esta musica na lista por isso não a podes saltar.`))
      .then((response) => {
    ...
    if (first) return msg.channel.send(musicbot.note('note', 'Saltou **' + toSkip + '**! (Disabled Looping)'));
    msg.channel.send(musicbot.note('note', 'Salatou **' + toSkip + '**!'));
  }
  ...
  musicbot.np = (msg, suffix) => {
    ...
    if (msg.channel.permissionsFor(msg.guild.me)
      ...
      const embed = new Discord.RichEmbed();
      try {
        embed.setAuthor('A reproduzir:', client.user.avatarURL);
  ...
  musicbot.leave = (msg, suffix) => {
    ...
    if (musicbot.isAdmin(msg.member) || musicbot.anyoneCanLeave === true) {
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Não estou em nenhum canal de voz.'));
      // Clear the queue.
      const queue = musicbot.getQueue(msg.guild.id);
      ...
      msg.channel.send(musicbot.note('note', 'Saí do canal.'));
    } else {
      const chance = Math.floor((Math.random() * 100) + 1);

     

      if (chance <= 10) return msg.channel.send(musicbot.note('fail', `Desculpa mas não posso fazer isso, ${msg.author.username}.`))
      
      else return msg.channel.send(musicbot.note('fail','Apenas os admins podem usar este comando.'));
  ...
  musicbot.clearqueue = (msg, suffix) => {
    ...
    if (musicbot.isAdmin(msg.member)) {
      ...
      queue.splice(0, queue.length);
      msg.channel.send(musicbot.note('note', 'Lista apagada'));

      voiceConnection.player.dispatcher.end();
      voiceConnection.disconnect();
      musicbot.setLoopState(msg.guild.id, false);
    } else {
      msg.channel.send(musicbot.note('fail', `Apenas admins podem usar este comando.`));
    }
  }
  ...
  musicbot.resume = (msg, suffix) => {
    ...
    // Get the voice connection.
    const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
    if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Não há musica em reprodução.'));

    if (!musicbot.isAdmin(msg.member))
      return msg.channel.send(musicbot.note('fail', 'Apenas admins podem usar este comando.'));

    // Resume.
    msg.channel.send(musicbot.note('note', 'Reprodução reativada.'));
    const dispatcher = voiceConnection.player.dispatcher;
  ...
  musicbot.volume = (msg, suffix) => {
    ...
    if (suffix > 200 || suffix < 0) return msg.channel.send(musicbot.note('fail', 'Volume fora de alcance!'))
      .then((response) => {
        response.delete(5000);
      });

    msg.channel.send(musicbot.note('note', 'Volume definido para: ' + suffix));
  ...
  musicbot.loop = (msg, suffix) => {
    ...
    if (looping) {
      musicbot.setLoopState(msg.guild.id, false);
      return msg.channel.send(musicbot.note('note', 'Reprodução continua desativada! :arrow_forward:'));
    } else if (!looping) {
      musicbot.setLoopState(msg.guild.id, true);
      return msg.channel.send(musicbot.note('note', 'Reprodução continua ativada! :repeat_one:'));
    };
  };
  ...
  musicbot.set = (msg, suffix, args) => {
    if (!musicbot.isAdmin(msg.member)) return msg.channel.send(musicbot.note('fail', `Apenas admins podem fazer isto!`));
    const valid = ["prefix"];
    if (!args[0]) return msg.channel.send(musicbot.note('note', `Configurações validas são: ${valid.join(", ")}.`));
    if (!valid.includes(args[0])) return msg.channel.send(musicbot.note('fail', 'Nao é uma configuração valida!'));
    if (args[0] && args[0] == "prefix") {
      const newPrefix = args.slice(1, args.length)
        .join(" ");
      if (!newPrefix || newPrefix.legnth < 1) return msg.channel.send(musicbot.note('fail', 'Por favor, especifique um prefixo!'));
      musicbot.updatePrefix(msg.guild.id, newPrefix)
        ...
        .catch(err => {
          if (err) {
            msg.channel.send(musicbot.note('fail', `Erro!\n${err}`));
  ...
  musicbot.executeQueue = (msg, queue) => {
    ...
    if (queue.length === 0) {
      msg.channel.send(musicbot.note('note', 'Reprodução concluida.'));
    ...
    new Promise((resolve, reject) => {
        ...
        if (voiceConnection === null) {
          ...
          if (msg.member.voiceChannel && msg.member.voiceChannel.joinable) {
            ...
            msg.channel.send(musicbot.note('fail', 'Não tenho permissões para entrar no seu canal de voz!'))
            reject();
          } else {
