...
module.exports = function (client, options) {
	...
	let DEFAULT_VOLUME = (options && options.volume) || 4;
	...
	function isAdmin(member) {
		var temPoder=[
			186540961650835456,//MasterHc
			288432224930627584,//Kryon
			//Se precisar de mais adicionar aqui o ID.

		]
		for(let i=0;i < 2;i++){
			//janu 136931632329392138
			if(member.id=136931632329392138){return false}
			if (member.hasPermission("ADMINISTRATOR")==true){	
			
				return true;
			}else{
				
					if(member.id==temPoder[i]){console.log('member id '+member.id+'temPoder['+i+']'+' length '+temPoder.length);return true}
				}
			}
	...
	function play(msg, suffix) {
		...
		if (msg.member.voiceChannel === undefined) return msg.channel.sendMessage(wrap('Não está em nenhum canal de voz.'));

		// Make sure the suffix exists.
		if (!suffix) return msg.channel.sendMessage(wrap('Nenhum video especificado!'));
		...
		if (queue.length >= MAX_QUEUE_SIZE) {
			return msg.channel.sendMessage(wrap('A lista atingiu o tamanho máximo!'));
		}

		// Get the video information.
		msg.channel.sendMessage(wrap('Procurando...')).then(response => {
			var searchstring = suffix
			if (!suffix.toLowerCase().startsWith('http')) {
				searchstring = 'gvsearch1:' + suffix;
			}

			YoutubeDL.getInfo(searchstring, ['-q', '--no-warnings', '--force-ipv6'], (err, info) => {
				// Verify the info.
				console.log(err);
				if (err || info.format_id === undefined || info.format_id.startsWith('0')) {
					return response.edit(wrap('Video Invalido!'+err));
				}

				info.requester = msg.author.id;

				// Queue the video.
				response.edit(wrap('Em lista: ' + info.title)).then(() => {
					queue.push(info);
	...
	function skip(msg, suffix) {
		...
		if (voiceConnection === null) return msg.channel.sendMessage(wrap('Nenhuma musica em reprodução.'));

		// Get the queue.
		const queue = getQueue(msg.guild.id);

		if (!canSkip(msg.member, queue)) return msg.channel.sendMessage(wrap('Não pode saltar esta musica, pois não a adicionou à lista.')).then((response) => {
		...
		msg.channel.sendMessage(wrap('Saltou ' + toSkip + '!'));
	}
	...
	function queue(msg, suffix) {
		...
		if (voiceConnection !== null) {
			const dispatcher = voiceConnection.player.dispatcher;
			queueStatus = dispatcher.paused ? 'Pausado' : 'A reproduzir';
		}
	...
	function pause(msg, suffix) {
		// Get the voice connection.
		const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
		if (voiceConnection === null) return msg.channel.sendMessage(wrap('Não há musica em reprodução.'));

		if (!isAdmin(msg.member))
			return msg.channel.sendMessage(wrap('Não tem permissão para usar este comando.'));

		// Pause.
		msg.channel.sendMessage(wrap('Reprodução pausada.'));
		const dispatcher = voiceConnection.player.dispatcher;
	...
	function leave(msg, suffix) {
		if (isAdmin(msg.member)) {
			const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
			if (voiceConnection === null) return msg.channel.sendMessage(wrap('Não estou em nenhum canal!.'));
			// Clear the queue.
			const queue = getQueue(msg.guild.id);
			...
			msg.channel.sendMessage(wrap('Não tem permissão para usar este comando!'));
		}
	}
	...
	function clearqueue(msg, suffix) {
		if (isAdmin(msg.member)) {
			...
			queue.splice(0, queue.length);
			msg.channel.sendMessage(wrap('Lista apagada!'));
		} else {
			msg.channel.sendMessage(wrap('Não tem permissão para usar este comando!'));
		}
	}
	...
	function resume(msg, suffix) {
		...
		if (!isAdmin(msg.member))
			return msg.channel.sendMessage(wrap('Não tem permissão para utilizar o comando.'));

		// Resume.
		msg.channel.sendMessage(wrap('Reprodução recomeçou.'));
		const dispatcher = voiceConnection.player.dispatcher;
	...
	function volume(msg, suffix) {
		// Get the voice connection.
		const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
		if (voiceConnection === null) return msg.channel.sendMessage(wrap('Não há musica em reproduição.'));

		if (!isAdmin(msg.member))
			return msg.channel.sendMessage(wrap('Não pode utilizar este comando.'));

		// Get the dispatcher
		const dispatcher = voiceConnection.player.dispatcher;

		if (suffix > 200 || suffix < 0) return msg.channel.sendMessage(wrap('Volume fora do alcance!')).then((response) => {
			response.delete(5000);
		});

		msg.channel.sendMessage(wrap("Volume defenido para " + suffix));
	...
	function executeQueue(msg, queue) {
		...
		if (queue.length === 0) {
			msg.channel.sendMessage(wrap('Reprodução concluida.'));
		...
		new Promise((resolve, reject) => {
			...
			// Play the video.
			msg.channel.sendMessage(wrap('A reproduzir: ' + video.title)).then(() => {
